...
-- index 삭제
alter table author drop index index명;
-- index 추가
...
-- 만약 where조건에서 2컬럼으로 조회시에, 2컬럼 각각에에 index가 있다면.
-- 이경우 db엔진에서 최적의 알고리즘 실행행 
select * from author where name='hong' and email='hongildong@naver.com';

-- index는 1컬럼 뿐만아니라 2컬럼을 대상으로 1개의 index를 설정하는것도 가능하다.
-- 이경우 2컬럼을 and조건으로 조회해야만 index를 사용
-- 복합 인덱스 생성
...
-- index테스트 시나리오
-- 아래 프로시저를 통해 수십만건의 데이터 insert후에 index를 생성 전후에 따라 조회성능 확인인
DELIMITER //
...
BEGIN
    ...
    WHILE iteration <= max_iterations DO
        ...
        WHILE i <= iteration * batch_size DO
            SET email = CONCAT('ronaldo', i, '@naver.com');
